...
class Terminfo
{
    ...
    public function readFile(string $file): Capabilities
    {
        ...
        $readInt16LE = static function (string $buffer, int $offset) {
            # s - signed short
            [, $value] = unpack('s', $buffer, $offset);

            return $value;
        };

        $readInt32LE = static function (string $buffer, int $offset) {
            # s - signed short
            [, $value] = unpack('l', $buffer, $offset);

            return $value;
        };

        $magic = $readInt16LE($buffer, $offset);

        if ($magic === 01036) {
            $extended = true;
        } elseif ($magic !== 0432) {
            throw new \RuntimeException(sprintf('Invalid terminfo magic number. Expected 0432 or 01036, got 0%o', $magic));
        } else {
            $extended = false;
        }

        // Read blocks sizes
        $namesSize = $readInt16LE($buffer, $offset + 2);
        $booleansSize = $readInt16LE($buffer, $offset + 4);
        $numbersSize = $readInt16LE($buffer, $offset + 6);
        $stringSize = $readInt16LE($buffer, $offset + 8);
        // $tableSize = $readInt16LE($buffer, $offset + 10);
        $offset += 12;

        // Read names
        $names = explode('|', substr($buffer, $offset, $namesSize - 1));
        $result['term'] = $names[0];
        $result['description'] = $names[1] ?? null;
        $offset += $namesSize;

        // Read booleans
        for ($i = 0, $max = min($booleansSize, self::$booleansCount); $i < $max; $i++) {
            [, $boolean] = unpack('h', $buffer, $offset + $i);
            if ($boolean) {
                $result['capabilities'][self::$booleans[$i]] = true;
            }
        }
        $offset += $booleansSize + ($offset + $booleansSize) % 2; // padded to short boundary

        // Read numbers
        $numberLength = $extended ? 4 : 2;
        for ($i = 0, $max = min($numbersSize, self::$numbersCount); $i < $max; $i++) {
            if ($extended) {
                $number = $readInt32LE($buffer, $offset + $i * 2);
            } else {
                $number = $readInt16LE($buffer, $offset + $i * 4);
            }
            if ($number !== -1) {
                $result['capabilities'][self::$numbers[$i]] = $number;
            }
        }
        $offset += $numberLength * $numbersSize;

        // Read strings
        $tableStart = $offset + 2 * $stringSize;
        for ($i = 0, $max = min($stringSize, self::$stringsCount); $i < $max; $i++) {
            $tableOffset = $readInt16LE($buffer, $offset + 2 * $i);
            if ($tableOffset !== -1) {
                $start = $tableStart + $tableOffset;
                $end = $tableStart + $tableOffset;
                while ($buffer[$end] !== "\0") {
                    $end++;
                }
                $string = substr($buffer, $start, $end - $start);
                $result['capabilities'][self::$strings[$i]] = $string;
            }
        }
